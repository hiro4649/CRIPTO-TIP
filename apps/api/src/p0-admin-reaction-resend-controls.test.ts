...
function manualPayload(requestId: string, moderation_status: "approved" | "hold" | "rejected" = "approved") {
  return {
    ...
    stream_id: "str_reaction_resend",
    character_id: "char_mio",
    display_name: "safe viewer",
    tier: "medium",
    message: "raw reaction resend message <script>",
    moderation_status
  };
}
...
function expectSafe(value: unknown) {
  ...
  expect(serialized).not.toContain("raw reaction resend message");
...
describe("P0 admin reaction resend controls", () => {
  it("requires admin bearer token and returns 404 for unknown support event", async () => {
    ...
    expect((await app.inject({ method: "POST", url: "/admin/support-events/missing/reaction-resend" })).statusCode).toBe(401);
    expect((await app.inject({ method: "POST", url: "/admin/support-events/missing/reaction-resend", headers: { authorization: "Bearer wrong-token" } })).statusCode).toBe(401);
    expect((await app.inject({ method: "POST", url: "/admin/support-events/missing/reaction-resend", headers: { authorization: adminAuth } })).statusCode).toBe(404);

    await app.close();
  });

  it("blocks held and rejected support events without enqueueing reaction resend", async () => {
    const repo = new InMemoryRepository();
    const app = buildServer(repo);
    await app.ready();
    const held = await createManual(app, "reaction_resend_hold", "hold");
    const rejected = await createManual(app, "reaction_resend_rejected", "rejected");

    const heldResponse = await app.inject({ method: "POST", url: `/admin/support-events/${held.event_id}/reaction-resend`, headers: { authorization: adminAuth } });
    const rejectedResponse = await app.inject({ method: "POST", url: `/admin/support-events/${rejected.event_id}/reaction-resend`, headers: { authorization: adminAuth } });
  ...
  it("enqueues one idempotent reaction resend candidate for approved support only", async () => {
    const repo = new InMemoryRepository();
    const app = buildServer(repo);
    await app.ready();
    const support = await createManual(app, "reaction_resend_approved", "approved");
    ...
    const first = await app.inject({ method: "POST", url: `/admin/support-events/${support.event_id}/reaction-resend`, headers: { authorization: adminAuth } });
    const second = await app.inject({ method: "POST", url: `/admin/support-events/${support.event_id}/reaction-resend`, headers: { authorization: adminAuth } });

    expect(first.statusCode).toBe(200);
    expect(first.json().status).toBe("queued");
    expect(first.json().side_effects).toEqual({ affinity: "skipped", reaction_request: "queued", overlay: "skipped", outbox: "queued" });
    expect(second.statusCode).toBe(200);
    expect(second.json().status).toBe("duplicate");
    expect(second.json().side_effects).toEqual({ affinity: "skipped", reaction_request: "duplicate", overlay: "skipped", outbox: "duplicate" });
    expect(repo.reactionRequests.size).toBe(before.reaction + 1);
    ...
    expect([...repo.outboxEvents.values()].filter((event) => event.idempotency_key === `reaction.resend:${support.event_id}:char_mio`)).toHaveLength(1);
  ...
  it("does not mutate support amount source wallet or moderation fields", async () => {
    const repo = new InMemoryRepository();
    const app = buildServer(repo);
    await app.ready();
    const support = await createManual(app, "reaction_resend_immutable", "approved");
    const before = await repo.getSupportEventById(support.event_id);

    const response = await app.inject({ method: "POST", url: `/admin/support-events/${support.event_id}/reaction-resend`, headers: { authorization: adminAuth } });
    ...
    expect(after?.support.message_moderation_status).toBe(before?.support.message_moderation_status);
  ...
  it("writes safe audit metadata only for reaction resend", async () => {
    const repo = new InMemoryRepository();
    const app = buildServer(repo);
    await app.ready();
    const support = await createManual(app, "reaction_resend_audit", "approved");

    await app.inject({ method: "POST", url: `/admin/support-events/${support.event_id}/reaction-resend`, headers: { authorization: adminAuth } });
    const audit = await app.inject({ method: "GET", url: "/admin/audit-logs?action=resend_reaction", headers: { authorization: adminAuth } });

    expect(audit.statusCode).toBe(200);
    expect(audit.json().audit_logs).toHaveLength(1);
    expect(audit.json().audit_logs[0].action).toBe("resend_reaction");
  ...
  it("committed reaction resend evidence preserves safe boundaries", () => {
    const evidence = readCodexEvidence("p0-admin-reaction-resend-controls.json");

    expect(evidence.adminReactionResendStatus).toBe("implemented");
    expect(evidence.adminAuthStatus).toBe("pass");
    expect(evidence.approvedResendStatus).toBe("pass");
    expect(evidence.heldResendBlockedStatus).toBe("pass");
    expect(evidence.rejectedResendBlockedStatus).toBe("pass");
    expect(evidence.resendIdempotencyStatus).toBe("pass");
    expect(evidence.noAffinityMutationStatus).toBe("pass");
    expect(evidence.noOverlayEnqueueStatus).toBe("pass");
    expect(evidence.noSupportMutationStatus).toBe("pass");
