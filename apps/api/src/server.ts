...
export function buildServer(repo: CriptoTipRepository = repository) {
  ...
  registerYouTubeFixtureRoutes(app, {
    repo,
    requireInternalAuth: (request) => requireBearer(request, INTERNAL_TOKEN),
    now: () => new Date().toISOString(),
    previewReactionDispatch: toReactionDispatchPreview,
    applySupportReceived: applySupportReceivedSideEffects
  });
