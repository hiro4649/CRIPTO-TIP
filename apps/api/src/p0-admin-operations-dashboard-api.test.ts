...
describe("P0 admin operations dashboard API", () => {
  ...
  it("committed PR evidence uses PR 76 GitHub evidence after PR creation", () => {
    const prBody = fs.readFileSync(path.join(root, "docs", "pr-p0-admin-operations-dashboard-api.md"), "utf8");

    expect(prBody).toMatch(/Head SHA: [0-9a-f]{40}/);
    expect(prBody).toMatch(/Base SHA: [0-9a-f]{40}/);
    expect(prBody).toMatch(/CI run: [1-9][0-9]+/);
    expect(prBody).toMatch(/Quality-gate run: [1-9][0-9]+/);
    expect(prBody).toMatch(/Quality-gate artifact: [1-9][0-9]+/);
    ...
    expect(prBody).not.toContain("artifact pending");
