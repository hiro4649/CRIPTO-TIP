...
export class InMemoryRepository implements CriptoTipRepository {
  ...
  async setReactionDispatchDryRunApproval(approval: ReactionDispatchDryRunApprovalMetadata) {
    this.reactionDispatchDryRunApprovals.set(approval.dry_run_boundary_id, approval);
    return approval;
  }
  async getReactionDispatchDryRunApproval(dryRunBoundaryId: string) {
    return this.reactionDispatchDryRunApprovals.get(dryRunBoundaryId);
  }
  async setReactionDispatchAdapterExecutionBoundaryApproval(approval: ReactionDispatchAdapterExecutionBoundaryApprovalMetadata) {
    this.reactionDispatchAdapterExecutionBoundaryApprovals.set(approval.dry_run_boundary_id, approval);
    return approval;
  }
  async getReactionDispatchAdapterExecutionBoundaryApproval(dryRunBoundaryId: string) {
    return this.reactionDispatchAdapterExecutionBoundaryApprovals.get(dryRunBoundaryId);
  }
  async setReactionDispatchLocalAdapterSimulationResult(result: ReactionDispatchLocalAdapterSimulationResultMetadata) {
    this.reactionDispatchLocalAdapterSimulationResults.set(result.simulation_result_id, result);
    return result;
  }
  async getReactionDispatchLocalAdapterSimulationResult(simulationResultId: string) {
    return this.reactionDispatchLocalAdapterSimulationResults.get(simulationResultId);
  }
