...
6. Update provider job state flags
7. Mark manual gate used with fail-closed `WHERE` predicates
8. Insert provider audit
9. Insert manual gate audit
10. `COMMIT`
...
Failures before provider success do not require compensation. Rollback failure
is reported as metadata-limited operator evidence and does not expose raw DB
diagnostics.
...
- No secret values, private URLs, wallet addresses, raw messages, display
  names, stdout, stderr, or stack traces are stored.
- No GitHub raw logs are read for this boundary.
