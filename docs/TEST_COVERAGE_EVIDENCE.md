...
- `apps/api/src/db-driver-approval-dry-run.test.ts` covers default `not_ready` evidence, selectedDriver null, owner approval missing, package change rejection, lockfile change rejection, added `pg` and `postgres` dependency rejection, changed script rejection, selected driver without owner approval, expired approval, target commit mismatch, branch mismatch, PR mismatch, fingerprint mismatch, missing preflight policy, missing license review, missing supply-chain review, missing advisory review, missing version pinning review, missing lockfile review, missing package diff review, missing secret boundary review, unsafe private URL/raw log evidence rejection, raw connection string rejection, readiness claim rejection, provider SDK apply rejection, production deployment rejection, test-only complete future fixture acceptance, and committed machine-readable evidence remaining not_ready.

## DB Driver Final Approval Gate v1.1.7 Prep

- `apps/api/src/db-driver-final-approval-gate.test.ts` covers committed `blocked` evidence, null selected driver, not-approved owner approval, not-ready readiness and approval dry-run, missing review statuses, all permission flags false, future complete fixture acceptance only in test code, committed rejection of approved/selected/ready states, committed rejection of any permission flag set to true, context mismatch rejection, unsafe evidence rejection, and proof that the future complete fixture is not committed as machine-readable evidence.

## DB Driver Dependency PR Template v1.1.7 Prep

- `apps/api/src/db-driver-dependency-pr-template.test.ts` covers default `template_ready` evidence, null selected driver, not-approved owner approval, blocked final gate, missing package/lockfile/license/supply-chain/security/version/secret evidence, all permission flags false, current template rejection of selected driver, dependency allowance, package/lockfile allowance, real DB connection, migration execution, live DB integration test, provider SDK apply, production deployment, readiness claims, legal and YouTube policy claims, required future PR sections, package diff rejection for multiple dependencies/scripts/unrelated changes, lockfile unrelated-change rejection, license compliance-claim rejection, raw log rejection, secret-boundary connection-string rejection, unsafe value/key rejection, future complete fixture acceptance only in tests, and proof that committed evidence does not include future package or lockfile evidence.
- Risk-to-test mapping: future dependency template mistaken for approval is covered by committed-evidence tests requiring null driver, not-approved owner approval, blocked final gate, and false package/dependency permissions; missing package/lockfile review evidence is covered by future-evidence tests requiring package diff, lockfile diff, license, supply-chain, security advisory, version pinning, and secret-boundary evidence; unsafe evidence is covered by raw log, raw provider response, DB connection string, token-like, private URL, wallet address, and secret-like key rejection tests; runtime or production overclaim is covered by runtime readiness, production readiness, legal compliance, and YouTube policy compliance rejection tests.
- Failure paths covered: selected DB driver in committed current evidence, approved owner/final gate in committed current evidence, package or lockfile permission flag enabled in committed current evidence, missing future package/lockfile review fields, raw logs or unsafe secret-like values in evidence, private URL evidence, and readiness overclaim evidence.

## DB Driver Candidate Review Pack v1.1.7 Prep

- `apps/api/src/db-driver-candidate-review-pack.test.ts` covers default `not_ready` evidence, `driverChoiceStatus: not_selected`, null selected driver, exact `pg`/`postgres` candidate list, per-candidate review status defaults, package/lockfile/dependency/DB/migration/live-test/provider/deployment/readiness/legal/YouTube policy permission flags false, selected driver rejection, ready-state rejection, pass/approved status rejection, missing/extra/duplicate candidate rejection, candidate summary overclaim rejection, unsafe value/key rejection, required section checks, blocker vocabulary checks, target/base SHA validation, and committed machine-readable evidence remaining not-ready.
- Risk-to-test mapping: candidate listing mistaken for approval is covered by selected/ready/approved rejection tests; future review completeness risk is covered by `not_reviewed`, `not_selected`, `missing`, and blocker vocabulary assertions; unsafe evidence risk is covered by DB connection string, private URL, wallet address, token-like value, unsafe run-output copy, provider response copy, console stream copy, trace copy, and secret-like key rejection tests.

## DB Driver Candidate Review Freshness v1.1.8 Prep

- `apps/api/src/db-driver-candidate-review-freshness.test.ts` covers default `not_ready` freshness evidence, exact `pg`/`postgres` candidate freshness entries, null selected driver, `driverChoiceStatus: not_selected`, `refreshRequired: true`, all missing review reasons, expiry policy constants, stale evidence policy constants, permission flag rejection, selected/fresh/current evidence rejection, timestamp rejection, missing/extra/duplicate candidate freshness rejection, unsafe value rejection, future fresh fixture isolation, and committed machine-readable evidence remaining not-ready.
- Risk-to-test mapping: stale evidence reuse is covered by freshness status, expiry, refresh reason, and timestamp rejection tests; accidental candidate selection is covered by selected driver, selected driverChoiceStatus, and summary wording rejection tests; package/dependency creep is covered by package, lockfile, dependency, real DB, migration, live DB test, provider SDK, production deployment, and readiness permission flag rejection tests.

## DB Driver Advisory Review Envelope v1.1.8 Prep

- `apps/api/src/db-driver-advisory-review-envelope.test.ts` covers default `not_reviewed` advisory evidence, exact `pg`/`postgres` candidate advisory review entries, null selected driver, `driverChoiceStatus: not_selected`, CVE/security advisory/package audit `not_reviewed`, `knownBlockersStatus: not_reviewed`, `knownBlockers: null`, `rawOutputPolicyStatus: safe_summary_only`, permission flag rejection, reviewed/pass/current evidence rejection, timestamp rejection, raw advisory/audit/dependency/log output rejection, unsafe value rejection, future reviewed fixture isolation, and committed machine-readable evidence remaining not-reviewed.
- Risk-to-test mapping: advisory envelope mistaken for approval is covered by review-ready/pass rejection tests; known blockers mistaken for a clean result is covered by empty-array rejection tests; raw output leakage is covered by raw advisory, raw audit, raw dependency tree, terminal output, raw log, private URL, DB connection string, wallet address, and token-like value rejection tests.
