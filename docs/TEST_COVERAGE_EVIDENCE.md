...
Latest local result: `corepack pnpm test` passed with 13 test files, 79 passed tests, and 6 skipped tests.
...
| Official YouTube API connector behavior. | `apps/api/src/youtube/connector.test.ts`, `apps/api/src/youtube/verification.test.ts`, `packages/shared/src/index.test.ts`. | Covered for adapter boundary and mock JSON fixtures; live API soak not covered. |
