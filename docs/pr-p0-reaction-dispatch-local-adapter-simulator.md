...
Adds a local-only adapter simulator for approved adapter execution boundary previews.

The simulator supports `success`, `retryable_failure`, and `terminal_failure` cases without real adapter execution.

## Evidence Integrity

Head SHA: current_pr_head

Base SHA: 134c769c19beab51644566f001986abfd61ee040
...
No dispatch attempt increment.
No support event, outbox, lease, attempt-plan, dry-run boundary, or real adapter status mutation during simulation.
