...
Dashboard apply now routes through the provider-safe deployment apply boundary. The dashboard-specific provider still owns dashboard plan application, but the shared boundary owns production-like gate validation, single-use marking, rollback reference enforcement, and safe audit evidence checks.
