...
| AC-CHAIN-001 | partial | `contracts/src/TipRouterV1.sol`, `apps/api/src/chain/tip-router-listener.ts`, `migrations/0002_chain_listener_reorg.sql` | TipSent decode, duplicate log, catch-up, subscription, cursor tests | Production RPC runtime wiring and process supervision remain future work. |
| AC-CHAIN-002 | partial | `apps/api/src/chain/tip-router-listener.ts`, `docs/RUNBOOK.md` | Confirmation window and reorg status transition tests | Operational metrics and deployment rollout remain future work. |
| AC-EVENT-001 | pass | `packages/shared`, `apps/api/src/repositories/*`, `apps/api/src/server.ts`, `apps/api/src/chain/tip-router-listener.ts` | Idempotency tests, live Postgres integration tests, support.normalize-after-confirmed test | Production IRIS delivery remains future work. |
