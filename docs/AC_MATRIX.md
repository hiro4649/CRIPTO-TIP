...
| AC-EVENT-001 | pass | `packages/shared`, `apps/api/src/repositories/*`, `apps/api/src/server.ts` | Idempotency tests | Live DB integration missing. |
...
| AC-AFF-001 | pass | `packages/shared`, `apps/api/src/repositories/*` | Affinity cap and idempotency tests | Live DB integration missing. |
...
| AC-ADMIN-001 | partial | `apps/api/src/server.ts`, `migrations/0001_durable_events.sql` | Admin auth and audit log tests | Admin DLQ retry endpoint not implemented. |
