...
export function renderPrEvidence(pack) {
  ...
  return [
    ...
    "",
    `Verification surface: ${(pack.verificationSurface || []).join(", ")}.`,
    "",
    `Risk surface: ${(pack.riskSurface || []).join(", ")}.`,
    "",
    `Oracle provided: ${pack.oracle || "Repository checks, evidence validators, and current-head GitHub checks."}`,
    "",
    `Split required: ${pack.splitRequired || "no - scoped evidence change."}`,
    "",
    `Best of N evidence: ${pack.bestOfNEvidence || "not required for this task profile."}`,
    "",
    `Review independence: ${pack.reviewIndependence || "writer and review evidence are separated in the rendered evidence pack."}`,
    "",
    "## Evidence Integrity",
    "",
